...
# CLASSE DA VARREDURA DO PJE DO DF DE SEGUNDO GRAU. HERDA OS METODOS DA CLASSE PJE
class DF2g(Pje2g):
    ...
    def busca_processo(self, numero_busca):
        ...
        while True:
            ...
            if footer.text == '0 resultados encontrados' or footer.text == '0 resultados encontrados.' or footer.text == 'resultados encontrados.':
                if self.proc_data['rec_status'] is not None and self.proc_data['rec_status'] == 'Segredo de Justiça':
                    raise FatalException("Erro ao buscar processo", self.uf, self.plataforma, self.prc_id, False)
                return False
